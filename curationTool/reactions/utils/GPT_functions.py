...
def parse_metabolic_reactions(text):
    ...
    for reaction_line in reaction_lines:
        # Split individual reactions into reactants and products using updated
        # regular expression
        reactions = re.split(
            r' <=> | = | <--> | → | -> | ⇌ | ↔ | <-> ',
            reaction_line)
        ...
        reactions = [reaction.strip()
                     for reaction in reactions if reaction.strip()]

        # Combine reactants and products into a single list and break down into
        # its constituent metabolites
        combined_reaction = ' + '.join(reactions)
        metabolites = combined_reaction.split('+')
        metabolites = [metabolite.strip()
                       for metabolite in metabolites if metabolite.strip()]
...
def parse_metabolic_reactions_gpt(text):
    ...
    for reaction_line in reaction_lines:
        ...
        if len(reactions) > 1:
            ...
            part1 = [metabolite.strip()
                     for metabolite in part1 if metabolite.strip()]
            part2 = reactions[1].split('+')
            part2 = [metabolite.strip()
                     for metabolite in part2 if metabolite.strip()]
...
def help_format_answer_with_gpt(raw_answer):
    ...
    question = "Given the answer: " + raw_answer + \
        ". Only give the chemical reaction(s) and nothing else i.e. no text. Start with '1.' indicating the first reaction. Only text not latex and use normal letters e.g. H2O instead of H₂O. Also remove electron charge e.g. NAD instead of NAD⁺ "
...
def metanetx_to_inchi(metabolite):
    x = metabolite
    api_url = "https://beta.metanetx.org/sparql/?default-graph-uri=https%3A%2F%2Frdf.metanetx.org.beta%2F&query=PREFIX+mnx%3A+%3Chttps%3A%2F%2Frdf.metanetx.org%2Fschema%2F%3E%0D%0APREFIX+owl%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F2002%2F07%2Fowl%23%3E%0D%0APREFIX+rdf%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F1999%2F02%2F22-rdf-syntax-ns%23%3E%0D%0APREFIX+rdfs%3A+%3Chttp%3A%2F%2Fwww.w3.org%2F2000%2F01%2Frdf-schema%23%3E%0D%0APREFIX+chebi%3A+%3Chttp%3A%2F%2Fpurl.obolibrary.org%2Fobo%2FCHEBI_%3E%0D%0ASELECT+%3Finchi%0D%0AWHERE+%7B%0D%0A++++%3Fmetabolite+a+mnx%3ACHEM+.%0D%0A++++%3Fmetabolite+rdfs%3Alabel+%3Flabel+.%0D%0A++++%3Fmetabolite+rdfs%3Acomment+%27" + \
        x + "%27+.%0D%0A++++%3Fmetabolite+mnx%3AchemRefer+%3Freference%0D%0A++++%0D%0A++++OPTIONAL+%7B+%3Fmetabolite+mnx%3Ainchi++++%3Finchi+%7D%0D%0A++++%0D%0A%7D%0D%0A%0D%0A%0D%0A&format=application%2Fsparql-results%2Bjson"
    params = {
        'format': 'json',
        'db': 'chem',
...
def evaluate_predictions(
    ...
    for ix, pred in enumerate(predicted_reactions):
        ...
        for idx, truth in enumerate(ground_truth_reactions):
            ...
            jaccard = jaccard_similarity(
                set(pred), len(
                    ground_truth_names[idx]), set(
                    flatten_extend(truth)))
    ...
    avg_jaccard = sum(max_jaccard_scores) / \
        num_predicted if num_predicted > 0 else 0

    if tracking:
        return {"Average Jaccard": avg_jaccard,
                "Over-prediction Penalty": over_prediction_penalty}, avg_jaccard, tracker

    else:
        return {"Average Jaccard": avg_jaccard,
                "Over-prediction Penalty": over_prediction_penalty}, avg_jaccard
...
def print_output(entrez_id, ix):
    ...
    print_statement, score, tracker = evaluate_predictions(
        gpt_predictions[ix], ground_truths_names[ix], ground_truths[ix], True)
    ...
    for idx, item in enumerate(ground_truths_names[ix]):
        ...
        for i, cid in enumerate(ground_truths[ix][idx]):
            ...
            print(item[i])
            html += '<a href="https://pubchem.ncbi.nlm.nih.gov/compound/' + \
                str(cid) + '" >' + str(cid) + '</a>' + " "
    ...
    for idx, item in enumerate(gpt_predictions_mid_and_metabolite[ix]):
        ...
        for y in item:
            ...
            if match:
                html += '<a style="color: green" id="' + mid + gene + \
                    str(idx) + ' " href="https://www.metanetx.org/chem_info/' + mid + '" >' + mid + '</a>' + ', '

            else:
                html += '<a style="color: red" id="' + mid + gene + \
                    str(idx) + ' " href="https://www.metanetx.org/chem_info/' + mid + '" >' + mid + ' </a>' + ', '
