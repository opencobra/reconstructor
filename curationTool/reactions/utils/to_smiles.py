...
def saved_met_to_smiles(id):
    ...
    return Chem.MolToSmiles(mol, allHsExplicit=True), None
...
def any_to_smiles(mols, types, request, side='substrates'):
    ...
    for idx, type in enumerate(types):
        if type == 'VMH':
            ...
            smile, error = saved_met_to_smiles(mols[idx])
            ...
            if request:
                ...
                smile, error = mdl_to_smiles(
                    temp_file_path, file_input_name=file_input)
            else:
                try:
                ...
                with NamedTemporaryFile(delete=False, suffix='.mol', mode='w+') as temp_file:
                    ...
                    temp_file.flush()
                smile, error = mdl_to_smiles(
                    temp_file.name, file_input_name='Temp Mol file')
        elif type == 'Draw':
